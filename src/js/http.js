...
const Http = (() => {
  ...
  function build(req, env) {
    const holder = { url: '' };
    const notes = [];

    // 1) URL：基础地址 + 路径变量 + query
    let base = Vars.resolve(req.url || '', env).trim();
    const parts = U.splitUrl(base);
    base = parts.base;

    for (const pv of req.pathVars || []) {
      if (!pv.key) continue;
      const val = encodeURIComponent(Vars.resolve(pv.value || '', env));
      base = base.replace(new RegExp(':' + pv.key + '(?![\\w-])', 'g'), val);
    }

    const paramItems = (req.params || []).map((p) => ({
      key: Vars.resolve(p.key, env),
      value: Vars.resolve(p.value ?? '', env),
      enabled: p.enabled
    }));
    ...
    for (const h of req.headers || []) {
      ...
      headers[Vars.resolve(h.key, env)] = Vars.resolve(h.value ?? '', env);
    }
    ...
    let body = { mode: 'none' };
    if (b.mode === 'raw') {
      body = { mode: 'raw', text: Vars.resolve(b.raw || '', env), contentType: b.rawType || 'application/json' };
    } else if (b.mode === 'urlencoded') {
      body = { mode: 'urlencoded', items: Vars.resolveItems(b.urlencoded, env) };
  ...
  /** 发送（自动处理 Token） */
  async function send(req, { onStage } = {}) {
    ...
    // 阶段 1：确保 Token 可用
    const needsToken = env && env.login && env.login.enabled && (req.auth || {}).type === 'inherit';
    if (needsToken && !Auth.isValid(env)) {
      stage('logging-in');
      const lr = await Auth.ensureToken(env);
      if (!lr.ok && !lr.skipped) {
